...
class GVS(eqx.Module):
    ...
    def _jacobian_derivative_gauss(
        ...
        def body_segment_i(carry, i_segment):
            ...
            def body_eval_points(carry, j_eval):
                ...
                eta_j = Ad_g_step_inv @ (eta_j + T_B_Magnus_step @ qd_i)

                Jd_j_scaled = Jd_j.at[:, :, 3:6, :].multiply(length_i)

                return (g_j, Jd_j, eta_j), Jd_j_scaled
    ...
    def resolve_upon_time(
        ...
        if u is None:
            u = jnp.zeros((self.dof_tot_system,))
        if tau_ext is None:
