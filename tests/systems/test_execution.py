"""Architecture tests for optional system execution backends."""
...
SOURCE_ROOT = Path(__file__).parents[2] / "src" / "soromox" / "systems"
    ...
    result = subprocess.run(
        [
            ...
            (
                ...
                "'._execution.warp.' in name for name in sys.modules)"
            ),
        ],
        check=False,
        capture_output=True,
...
def test_warp_execution_does_not_import_system_implementations() -> None:
    ...
    warp_root = SOURCE_ROOT / "_execution" / "warp"
    sources = "\n".join(
...
def test_system_models_only_import_the_neutral_execution_api() -> None:
    ...
    assert "soromox.systems._execution.warp" not in model_sources
...
def test_warp_families_only_share_through_common_modules() -> None:
    ...
    gvs_sources = "\n".join(
        ...
        for path in (warp_root / "gvs").rglob("*.py")
    )
    pcs_sources = "\n".join(
        path.read_text(encoding="utf-8")
        for path in (warp_root / "pcs").rglob("*.py")
    )
    common_sources = "\n".join(
        path.read_text(encoding="utf-8")
        for path in (warp_root / "common").rglob("*.py")
    )
    assert "soromox.systems._execution.warp.pcs" not in gvs_sources
    assert "soromox.systems._execution.warp.gvs" not in pcs_sources
    assert "soromox.systems._execution.warp.gvs" not in common_sources
    assert "soromox.systems._execution.warp.pcs" not in common_sources
