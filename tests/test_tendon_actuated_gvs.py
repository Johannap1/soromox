...
def test_actuation_matrix_gvs():
    ...
    for segment_lengths, tendon_params in test_cases:
        ...
        if segment_lengths.shape[0] == 1:
            ...
            basis1 = BasisAttributes(basistype="Monomial", Bdof=[1, 1, 1, 1, 0, 0], Bodr=[1, 1, 1, 1, 0, 0])
            basis2 = BasisAttributes(basistype="Monomial", Bdof=[1, 1, 1, 1, 0, 0], Bodr=[0, 0, 0, 0, 0, 0])
...
def test_tendon_length_gvs():
    ...
    for segment_lengths, tendon_params in test_cases:
        ...
        if segment_lengths.shape[0] == 1:
            print("The lenght coincide with the length of the hypotenuse of the triangle formed by the segment length and 2*ry_distance.") 
            hypotenuse = onp.sqrt((segment_lengths[0] ** 2 )+((2* tendon_params["ry"][0]) **2 ))
            assert_allclose(
            l_tendons,
            hypotenuse,
            rtol=Tolerance.rtol(),
        ...
        assert not jnp.isnan(l_tendons).any(), (
            "Tendon lengths contains NaN!"
        )
...
def test_tendon_actatuated_gvs_vs_pcs():
    ...
    for segment_lengths, tendon_params in test_cases:
        ...
        if segment_lengths.shape[0] == 1:
            ...
            joint1 = JointAttributes(jointtype="Fixed")
            basis1 = BasisAttributes(basistype="Monomial", Bdof=[1, 1, 1, 1, 0, 0], Bodr=[0, 0, 0, 0, 0, 0])
        ...
        if segment_lengths.shape[0] == 1:
            ...
            print("PCS Actuation matrix A (scaled for consistency):\n", A_PCS)
            print("No contributions from the second segment.")
        assert not jnp.isnan(A_GVS).any(), (
            "Actuation matrix contains NaN!"
        )
        assert_allclose(
            A_GVS,
